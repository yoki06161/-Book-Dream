...
public class UserController {
	...
	public String modifynameform(@Valid UserModifyNameForm userModifyNameForm, Model model, Principal principal) {
		SiteUser user = null;
		Member member = null;
...
//        try {
//            user = this.userService.getUserByEmail(principal.getName());
////            pay = (Pay) this.payService.getPaysById(principal.getName());
//        } catch (DataNotFoundException e) {
//            // user가 없으면 무시하고 member를 찾기 위해 진행
//        }
//
...
////            model.addAttribute("name", member.getName());
////            List<Pay> payList = this.payService.getPayByEmail(member.getEmail());
////            if (pay.getEmail().equals(member.getEmail())) {
////                isBuy = true;
...
//        } else if (user != null) {
//    		Prod_Books book = prodService.getProdBooks(book_id);
//    		model.addAttribute("book_title", book.getBook_title());
////            model.addAttribute("name", user.getUsername());
////            List<Pay> payList = this.payService.getPayByEmail(user.getEmail());
////            if (!payList.isEmpty()) {
////                isBuy = true;
////                model.addAttribute("isBuy", isBuy);
////                model.addAttribute("payList", payList);
...
//    }
	...
	public String userbuy(Model model, Principal principal) {
		...
		List<Pay> payList = payService.getPayByEmail(email); // 이메일로 결제 내역을 조회합니다.
